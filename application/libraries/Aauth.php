...
//				$this->send_verification($user_id);
	...
	public function delete_user($user_id) {
		...
		return $this->aauth_db->delete($this->config_vars['users']);
                ...
                //delete employee 
                $this->aauth_db->where('emp_id', $user_id);
		return $this->aauth_db->delete($this->config_vars['employee']);
	...
	public function get_user_groups($user_id = FALSE){
		...
		if( !$user_id){
			$this->aauth_db->where('name', $this->config_vars['default_group']);
 ...
 public function get_group($perm_par) {

        if (is_numeric($perm_par)) {
          $query = $this->aauth_db->where('id', $perm_par);
        }else{
	...
	public function is_group_allowed($perm_par, $group_par=FALSE){
		...
		// checks current user's all groups
			...
			if ( $this->is_admin( $this->CI->session->userdata('id')) OR
				$this->is_group_allowed($perm_id, $this->config_vars['moderator_group']) )
			{return TRUE;}
	...
	public function list_perms() {
		...
		return $query->result();
         ...
         public function assigned_perms() {
